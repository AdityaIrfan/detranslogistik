                                ...
                                <tr>
                                    <td class="text-center">
                                        <a href="/articles/{{ $article->image }}">
                                            <img class="img-responsive" alt="{{ $article->title }}" style="width:150px;" src="/articles/{{ $article->image }}" >
                                        </a>
                                    </td>
                                    <td class="text-center">{{ $article->title }}</td>
                                    <td>{!! $article->content !!}</td>
                                    <td class="text-center">
                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('article.delete', $article->id) }}" method="POST">
                                            <a href="{{ route('article.getUpdatae', $article->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                            @csrf
                                            {{ method_field('DELETE') }}
